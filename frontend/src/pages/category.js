      ...
      <PageWrapper>
        <NavBarWrapper>
          ...
          {show ? <ArrowBackIosIcon
            ...
            onClick={() => {setShow(false);setKeywords('')}}
          /> : ''}
        ...
        <Navpagewrapper>
          ...
          {show ? <Box sx={{ width: '100%', maxWidth: '1000px' }}>
            ...
            />)}
          </Box> : <Box sx={{ flexGrow: 1, width: '100%', display: 'flex' }}>
          <Tabs
        ...
        <Box sx={{ display: 'flex'}}>{active.list.map((ele, index) => <Typography sx={{ px: '16px', py: '8px'}} onClick={() => setKeywords(ele)} key={index}>{ele}</Typography>)}</Box>
        </Box>}
          </Box>
