...
router.route("/:id").delete(checkAuth, checkObjID, async (req, res) => {
  try {
    ...
    if (!recipe) {
      ...
      res
        .status(403)
        .json("Recipe deletion failed, user is not the recipe author");
    }
  } catch (err) {
    //unknown error
    res.status(500).json("Error: " + err);
...
router.route("/update/:id").put(checkAuth, checkObjID, async (req, res) => {
  ...
  try {
    ...
    if (!recipe) {
      ...
      res
        .status(403)
        .json("Recipe update failed, user is not the recipe author");
    }
  } catch (err) {
    //unknown error
    res.status(500).json("Error: " + err);
...
router.route("/comments/:id").get(checkObjID, async (req, res) => {
  ...
  try {
    ...
    const query = await Recipe.findById(req.params.id, projection).populate(
      "comments.userId",
